...
import com.sumeyyesahin.firebasekotlin.view.DetailActivity
...
class RvAdapter(private var productList: List<Product>) :
    ...
    override fun onBindViewHolder(
        holder: ViewHolder,
        position: Int
    ) {// anlamı: tutucuyu bağla, pozisyonu al
        val currentItem =
            this.productList.get(position) // anlamı: bu ürün listesinin bu pozisyonundaki ürünü al
        ...
        holder.binding.apply { // anlamı: tutucuya uygula
            textView.text =
                currentItem.title // anlamı: tutucudaki metin alanına bu ürünün başlığını yaz
            Picasso.get().load(currentItem.thumbnail).error(R.drawable.noimage)
                .into(imageView) // anlamı: Picasso ile bu ürünün resmini al, hata olursa noimage koy, imageview'e koy
            textView2.text =
                "Fiyat: ${currentItem.price.toString()} TL" // anlamı: tutucudaki metin alanına bu ürünün fiyatını yaz
        ...
        checkBox.setOnCheckedChangeListener { checkBox, isChecked -> // anlamı: kalp kutusuna tıklandığında
            if (isChecked) {
                ...
                database.child(currentItem.id.toString() + currentUser!!.uid) // anlamı: veritabanında bu id ile bu kullanıcıyı al
                    ...
                    ) // anlamı: bu kullanıcı id'si ile bu ürün id'sini veritabanına ekle
        ...
        holder.itemView.setOnClickListener { // anlamı: tutucuya tıklandığında
            ...
            ) // anlamı: intent ile bu tutucunun bağlı olduğu context'te DetailActivity'e git
